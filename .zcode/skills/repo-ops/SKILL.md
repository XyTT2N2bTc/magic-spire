...
- 规则类文档引用检查：`& tools/check-docs.ps1`（改动 `docs/spec`／`docs/design`／`docs/guide`／根 `AGENTS.md`／`.zcode/skills` 后必跑；检查点名路径存在、`文件::符号` 锚点已声明、本地 md 链接可达，并打印允许清单条数；扫描范围与排除理由的唯一声明在 `tools/doc-scan-scope.ps1`，`-ListTokens` 逐条打印）。这些规则类文档同时在源码指纹内：改动它们会触发 `SOURCE CHANGED`。
