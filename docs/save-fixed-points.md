...
| --- | --- | --- | --- | --- |
| P1 | **进入新的一层** | **仅**：`state` 的当前房间 `floor` ≠ `original` 的当前房间 `floor`（`room_data(state.room).floor`／`room_data(original.room).floor`），**任一方向都算**（上行、下行都触发）；在 `dispatch` 成功、提交后。**换塔／出狱返塔不写盘**（人审裁定，见 §10） | 提交后的 `restart_snapshot()` ＋ 当时线稿 | **该层入口**（新层第一个房间的起点） |
...
3. **换塔／出狱返塔是否算"进入新的一层"——已裁定：不算。** P1 已删去 `tower_generation` 子句，条件仅为
   "提交前后当前房间 `floor` 不同"；换塔不写盘，恢复点仍为**上一次固定点**（反例清单加 N12）。
   **待否决的解释（人审记录补充）**：`floor` 变化按**任一方向**都算（上行也触发），因为这最贴近
   "进入新的一层"的字面；若人反对，改条件即可（例如只算下行、或只算上行）。
