...
2026-09-19内容包根全局开关：`core/content_catalog.gd` 不再按构建类型判断内容包目录，原 `OS.has_feature("editor") or OS.has_feature("android")` 判定整行删除。新顺序固定为 显式 `--packs-root=<路径>` > EXE 旁 `content/packs`（存在时）> `res://content/packs`，结果只算一次并缓存（`packs_root_cache`），`ensure()` 是唯一消费者；`--packs-root` 也只有一个读取点（引擎参数与 `--` 之后的用户参数都读，覆盖启动脚本与工具／测试两种写法）。Android 因此与桌面同路：包内没有 EXE 旁目录，走 `res://` 回退。启动脚本 `tools/launch.ps1` 与未入库的 `build/play.ps1` 显式传 `--packs-root=<游戏目录>/content/packs`；`docs/spec/packaging.md` 收一句探针顺序（唯一正文），`docs/spec/project-map.md` 目录表加指针，不在多处重复。四条检查（开关优先／相邻命中／回退／无 `has_feature` 静态断言）落在 `tests/content_cases.gd` 与 `tests/architecture_cases.gd`；规则门4套件1560断言退出码0、指纹未漂移，敏感性两条实测分别打红①②与③。未跑 oracle／像素／性能与打包探针（按人指示留到定稿轮），未打包、未推送。详见 docs/record/verification.md。

2026-09-19内容包根改回常量（取代同日上一条「内容包根全局开关」）：`core/content_catalog.gd` 删除 `--packs-root=` 开关解析（`packs_root_switch()`）、EXE 旁存在性探针顺序（`resolve_packs_root()`）与缓存（`packs_root_cache`），改为常量 `PACKS_ROOT := "res://content/packs"`（开发值：编辑器、测试与 Android 包内资源）＋唯一访问器 `packs_root()`（`"adjacent"` → 可执行文件旁的 `content/packs`；其它值原样使用）。发布值由打包时手工翻转：`tools/package.ps1` 导出前断言 `"adjacent"`、`tools/package-android.ps1` 断言 `"res://content/packs"`（共用 `tools/assert-packs-root.ps1`，失败消息给出文件与要改的整行，不自动改；Android 的 packs 打进 APK 经 `res://` 读取，翻成 `"adjacent"` 反而丢内容，故断言方向与 Windows 相反）。`tools/launch.ps1` 不再传 `--packs-root`；`docs/spec/packaging.md`「域」收取值、翻转步骤与断言的唯一正文，`docs/spec/project-map.md` 只留指针。检查：`tests/content_cases.gd::packs_root_single_switch`（常量值为开发值／非 adjacent 值原样返回／adjacent 解析式）与 `tests/architecture_cases.gd::content_pack_root_has_no_build_feature_branch`（无 `has_feature`、单一常量＋访问器、开关／探针／缓存不得回归、core・ui・data 只有 content_catalog.gd 拼内容包路径）更新，镜像用例删除；规则门4套件1563断言退出码0、指纹未漂移；翻转常量敏感性实测 content 5 条红、architecture 2 条红。未跑 oracle／像素／性能与真实打包（打包断言以一次性 pwsh 探针跑四例正反例），未打包、未推送。详见 docs/record/verification.md。
