...
- **计数注意**：分类断言数不是恒定判据。本片窗口 `route` 253（上一批登记 156，本片新增 97 条）、`localization` 窗口 51 与规则 164、`architecture` 479 与上一批逐数相同；`interface` 旁证 404 与 2026-09-19 登记相同。计数只作范围佐证，判据仍是具名 check ＋分类 PASS／FAIL ＋本轮实际计数。
