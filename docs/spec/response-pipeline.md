...
| M2 提交 | `ui/main.gd` 的 `commit` | `commit(c, expected_version) -> Dictionary` | 分流、守卫、反馈编排 |
...
| `ActionIndex` | `_init(actions)` 只建 `by_id`／`by_group`；`select` 按 payload 相等筛选；`find` 取首个匹配（无匹配 `{}`）；`first_usable` 全不可用返回**末项** | 构造点唯一：View 更新时（`present`／`render`） | 登记在 `by_id` 的候选就是该 View 的候选；不保证提交成功（由 `dispatch` 复核）；陈旧索引只允许用于"按旧版本提交并得到拒绝"的测试路径 |
...
`ui/main.gd` 的现状实现入口是 `_submit`（整树 `render(view)`）；本契约把提交与落地拆成下面三个接口，
`_submit` 可实现为 `commit` 的薄别名。
...
- 拒绝呈现：打回即打回，不新增动画、最多留一个触发器；被拒路径零动画（反馈四调用整体在成功
...
- 禁止：版本不等时跳过重同步或跳过 `actions` 替换；任一情形吞掉 `notice`；自动重试；删掉上述断言；
  以"应该更快"或单次采样宣称提速。
- 成功路径不得因本契约改动：`_save_progress` 仍只在 `ok`；`expand_applied` 的 phase 条件不变；
...
- 具名 check（判据＝测试侧布尔断言；计数只用测试侧包装，生产源码不带计数器；场景编号被其他契约引用，
  不得重排）：
