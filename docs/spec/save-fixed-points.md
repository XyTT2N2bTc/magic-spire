...
- 文件域：`core/game.gd`（声明表的 `checkpoint` 标记、`_checkpoint_kind`、`dispatch` 结果的
  `checkpoint` 键）、`ui/main.gd`（写盘判据与三条非进度写盘）、`core/save_store.gd`（**不改**）。
- 人可见后果（人已裁定接受）：
...
- 非目标：不改 `core/save_store.gd`、不改存档格式与 `save_revision`、不改 `pack()`／`unpack()` 的
  格式与校验、不改 `restart_snapshot()` 的冻结时机、不改 `read_slot` 回退规则与 `summary()` 语义、
  不改 `TRANSITIONS` 既有 kind 语义（只加 `checkpoint` 标记）、不新增生产文件与只读接口、
  不新增模块依赖、不做启动期迁移脚本、不改启动链。
