...
| 文件 | 允许的改动 | 必须保持 |
| --- | --- | --- |
| `ui/run_review.gd`（新建） | 新建 `extends RefCounted` 的静态组装入口：`TITLE_KEY`／`TITLE_FALLBACK`／`title_text(ui)`／`can_open(route)`／`drawer(ui)` 与三块内容（标识／节点概况／卡组） | 只读 `ui.view`、`ui._text`、`ui._label`、`ui._scroll`、`ui._drawer_shell`；自身不调用任何 `ui.game.*`；不派发候选、不进 `_submit`、不写盘 |
...
| `ui/main.gd` | `DRAWERS` 增 `show_run_review`；`var show_run_review=false`；`const RunReview=preload("res://ui/run_review.gd")`；`_refresh_drawers()` 非主页分支增一行分派；`_drawer_shell()` 的整窗遮罩条件增 `show_run_review`；`_route_screen()` 的 `navigation` 增 `OpenRunReview`；`_demo_exit_screen()` 增 `OpenRunReview`（其面板 rect 高度可按内容调整） | `SeedChip`、`seed_report_text()`、`copy_seed()`、`_seed_chip_text()` 不动；`MapOverview`／`MapLocate`／`MapClearDrawing`／`TowerRoute`／`TowerMapScroll` 的文本、位置、行为与相互判据不动（本片只**追加**一个按钮）；`_select_route_room`、`_open_drawer`／`_close_drawers`、Esc 与安卓返回键清单语义不变 |
| `assets/localization/zh_CN.json` | 新增「本地化 key 表」的 7 条（`text`＋`context`） | 既有条目不改不删；`schema_version`／`locale` 不变 |
| `assets/localization/en_US.json` | 新增同 7 条（`source` 与 zh_CN 的 `text` 逐字相同，`text` 非空） | 既有条目的 `source`／`text` 不改；`coverage("en_US").missing==0` 继续成立 |
| `tests/route_ui_cases.gd` | 新增 `run_review(t)` 与场景 A–I 的具名 check（在既有 `run(t)` 恢复 `ui.saves`／`persistence_enabled` 之前调用） | 既有断言与既有 `merged_departure`／`seed_chip` 不删不改；不新增镜像测试；分类注册（`tests/ui_smoke.gd` 的 `UI_MODULES`）不动 |
...
- 在回顾面板里调用 `ui._submit`／`ui.actions`／`ui.game.dispatch`／`ui._save_progress`／`ui.copy_seed`；
...
- 两份本地化新增条目不重叠（`ui.map.seed*`／`ui.feedback.save.*` 与 `ui.run_review.*`）；
  同一 PR 内先后运行门禁时，`coverage("en_US").missing==0` 是两条片共同维持的判据。
