...
完整随机域清单与盐值只维护在 `data/balance.gd::RNG_SALTS`；Game 初始化按该表创建全部零计数，Snapshot 按同表要求“完整且无额外域”，每个计数为非负整数。新增域必须登记该表并覆盖实际执行与保存恢复；破坏存档结构时提升 `Snapshot.REVISION`，不迁移或补填旧档。不要借显示代码或全局 `rand*` 增加不可重放的抽取。
