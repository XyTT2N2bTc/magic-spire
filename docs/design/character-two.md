...
- 四部位施法预备在高潮或本场整备结束时清空；“耐心耐心～”生效期间施法预备不会减少（主动释放除外），也不能用预备抵挡拘束。保护覆盖本次行动结束后的敌方行动，在下回合开始、回合开始的装备效果结算前失效；本场结束仍照常清理。
...
flowchart LR
 ...
 E --> F[回合开始装备效果及后续行动]
```
- 精神集中：下一次造成魔法伤害时每层使各段基础伤害＋1，整次用完全部层数，魔力松缚不受此加成，也不消耗精神集中；施法失败或高潮时失去1层。本场整备结束最多保留2层（乌龟壳提高到4层），场内不限层数。
- 魔术手自由面在该角色下为闪避2（不再忽略手部拘束）。

## 4. 初始11张牌

3张顾涌！（魔女）、3张用力！、1张魔力松缚、1张脱缚练习，以及三张专属基础牌（不随机出现、不入奖励池）。顾涌！（魔女）双面效果沿原牌；用力！自由面为精神集中1。

| 卡牌 | 费用 | 效果 |
| --- | --- | --- |
| 魔法钥匙 | 1能量，精神施法 | 自由面：获得4层魔力预备；拘束面：开锁1 |
| 施法预备 | 1能量、20魔力，精神施法 | 双面：获得4层魔力预备、2层精神集中 |
| 魔力积蓄 | 2能量，能力 | 双面：每点当前自身与临时魔力使造成伤害＋1% |

- 魔力积蓄按实际造成伤害时的自身与临时魔力总和计算，支付耗魔后再计算攻击加伤，与其他伤害倍率相乘；能力持续至整备结束，可叠加。
- 脱缚练习两面分别挣扎／滑脱，单张实体牌两面共享本局累计次数，整张结算完成只计1次，跨战斗保留。鼠标悬浮简介显示该实体牌的累计／下一门槛次数、距离升级次数；35次起显示全部升级完成。第7／14／21／28／35次用完后永久进化，当前这次仍按出牌时阶段逐段结算，进化后永久卡组与各牌堆实体一致。

| 阶段 | 每段伤害×段数 | 费用 |
| --- | --- | --- |
| Lv.0 | 1×3 | 1能量 |
| Lv.1（7次后） | 1×6 | 1能量 |
| Lv.2（14次后） | 2×6，顺延 | 1能量 |
| Lv.3（21次后） | 3×6，顺延 | 1能量 |
| Lv.4（28次后） | 3×6，超级顺延 | 1能量 |
| Lv.5（35次后） | 3×6，超级顺延 | 0能量 |

×表示独立段数，每段复核目标与乘区；顺延复用同部位→同大部位→同区域规则，超级顺延在本区域无合法目标后可转向全身其他合法区域；图鉴同页列出各进化阶段。所有阶段均不额外清除捕缚。
...
| 牌 | 自由面 | 拘束面 |
| --- | --- | --- |
| 魔术手（魔女）（含遗物赠牌） | 1能量、30魔力，闪避2 | 1能量、30魔力，降紧4，超级顺延 |
| 魔力松缚 | 魔力预备2（仍抽1） | 保留 |
| 汲取（魔女）·普通 | 恢复10魔力，抽1 | 恢复10魔力，消耗 |
| 预备咏唱 | 双面0能量，其余沿原牌 | 同左 |
| 蓄势待发 | 下一次基础动作费用−2 | 精神集中3 |
| 魔路检索 | 双面0能量、5魔力，消耗，精神施法 | 同左 |
| 激发魔力 | 恢复10魔力，魔力预备4 | 同左 |
| 魔力涌流 | 保留 | 0能量、20魔力，精神集中2 |
| 灵活变通（魔女） | 每回合开始魔力预备2 | 每回合开始精神集中1 |
| 魔力转换 | 恢复20魔力 | 消耗20魔力，能量＋2 |
| 找准松处 | 保留 | 精神集中1，仍抽1 |
| 魔力回路 | 保留 | 每累计耗魔20，精神集中1 |
| 欲能转换（魔女） | 每15快感获得1能量 | 同左 |

蓄势待发双面仍须选择并消耗1张手牌，仅施法成功消耗；减费最低0、一次用尽。

| 专属新牌 | 自由面 | 拘束面 |
| --- | --- | --- |
| 绘制法阵·罕见能力·1费 | 基础动作施法预备不再消耗魔力；上身拘束等级≤3；释放费用不变 | 下3张技能牌费用−1，最低0；0费技能也消耗次数，再次使用累计次数，不叠加单张减费幅度 |
| 魔力抽调·普通 | 1能量无部位魔法，获得4层魔力预备 | 0费技能，至多消耗40魔瓶魔力等量恢复自身（按瓶余量与缺口封顶） |
| 耐心耐心～·罕见·保留 | 1能量、20魔力，无部位魔法，嘴部装备等级＜3，下回合额外3能量 | 0费技能，直到下回合开始施法预备不会减少（主动释放除外），暂停预备抵挡拘束；高潮保留预备 |
...
自由面通过现有能力列表查询预备免魔力；四部位共用基础动作候选费用入口，只改变预备形态。拘束面通过现有卡牌增益剩余次数记录，实际牌面为技能才减费并消耗次数，失败或过期候选不消耗。X费技能沿既有实际支付量作为X的语义，减费后留下1能量；零费技能仍消耗一次。未用完次数跨回合保留，整备结束统一清理，两面可共存。

```mermaid
flowchart LR
 A[绘制法阵进入能力区] --> B[自由面能力查询]
 A --> C[拘束面授予3次技能减费]
 B --> D[基础动作候选: 预备魔力为0]
 C --> E[卡牌费用查询: 当前技能面减1]
 E --> F[正式提交: 支付并消耗1次]
