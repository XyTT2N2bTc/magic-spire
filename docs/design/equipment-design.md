...
- 马眼全包榨精杯：中级款覆盖柱身、龟头、冠沟与马眼，最大耐久16、有限电量9回合；每次消耗能量基础快感8，玩家回合开始基础快感18。高级款沿用既有数值。
...
```mermaid
flowchart LR
  Predicate[SpecialEquipment.unlocked_release 共用判定] --> Preview[Game.escape_preview 正式候选]
  Predicate --> Hit[Game._apply_equipment_damage]
  Predicate --> View[ReleaseView.preview]
  Preview --> Commit[原候选 ID 与版本提交]
  Commit --> Hit
  Hit --> Cleanup[原统一清理：锁体与所属加固带]
```
...
```mermaid
flowchart LR
 A[敌人／事件／卡牌装备请求] --> B[原装备候选与安装准备]
 B --> C[Equipment.capacity: 总容量与来件资格]
 C --> D[Game._prepare_installation: 混材胶带置外]
 D --> E[原工厂写入真实layer]
 E --> F[外露判定／解除候选／界面层序]
 C --> G[获准替换: 原最外层及品质比较]
 G --> D
...
```mermaid
flowchart LR
 A[杯类类型] --> B[exclusive_family: 同一佩戴组]
 B --> C[安装准备: 已有则拒绝追加]
 B --> D[获准替换: 原强度比较与同批保护]
 B --> E[存档聚合校验]
 F[旧档迁移: 品质优先／同品最早] --> G[清理额外主体与所属带／监狱登记]
 G --> E
```
