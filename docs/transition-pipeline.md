...
- **判据身份（§0.1 D4；已登记 `77d8dd7`）**：**比对是门禁、摘要是脚本版本指纹**。
  - 冻结时：脚本 sha256 `b49b0164a6b962fc8eae4a843c36510e1fa12c846d9f0e565856fdc6ed092278`，
    摘要 `00089c29a675e1268473645ab6b7a363e70295abf575cc6cc2929db995ca3e11`；
    当前（补全判据身份后）：脚本 sha256 `59d41c682b3b302c1d291eb64aea770994c89508ee3fcb637afc7902024c045c`，
    摘要 `14eb8cf9c3c8b5d4347b2b9d118b8c504596e04d296d091884995bc359b522b6`（两次运行同值）；
    **基线文件 sha256 `ba979d18c31952d6d69ef06ce2ed102f7503c518fa8c6125ea6482c92bb5b4c8` 自冻结起未改**。
  - **口径**：**重跑时摘要不一致不等于行为漂移**——必须**先核对身份**（脚本指纹＋基线哈希＋比对结果）
    再判定；两组数字都登记在 `docs/verification.md`。
  - **抓取路径不变性证据（已补）**：当前脚本 `--write=` 的输出与冻结基线**行名与逐行键集完全相同**，
    唯一差异列是 `transition_log`；两侧投影到参与比对的六列
    （`before/after/commit_logs/log_texts/digest/transition_log_declared`）规范化后**同为
    sha256 `5333ab642896dd5932faca53211aa7f77510b59065fc0c3a0a1d6374bd78d2fb`**。
- **折叠边界（人审裁定接受，写进契约而非只留报告）**：`_compare_row` **仅对 `transition_log` 一列**
  双侧折叠相邻同名（放过"同一 kind 相邻重复的额外一条"）；**其余列仍逐字段硬比对**，
  并保留 `NEWFIELD／MISSINGFIELD` 双向检查。
  - **接受理由**：该列是**进程内诊断**（不进 `state`／存档／View）；"恰一条 `battle_end_*`／均为
    `prepare_end`"这类**语义**由闭环 check 与场景 01／02／04 承接，不由 oracle 的列比对承接。
  - **未放宽比对的证据**：六列投影两侧同为 `5333ab64…`；冻结脚本自比（`selfcheck.log`）31 条差异
    **全在 `transition_log`**、行为字段 0 条。
  - **附加条件（已收口）**：该日志**现在已成为存档切片的数据源**（`checkpoint` 由 `_transition_log`
    的本次增量推导）。收口结论见 `docs/save-fixed-points.md` §6：**本片可接受该边界**，
    但要求"推导按类别、与条目数无关"＋"语义由闭环 check 与具名场景承接"＋"审计留痕"三条同时成立；
    若将来需要该列**逐条精确**的诊断，必须**另立批次**恢复严格比对。
...
## 9. 收口状态与验收建议（2026-09-16；§9 原"需人确认"章节内容已由人审与执行满足）

**批次表（全部完成）**：基线冻结 `9ee7a2f`／战斗结束 `d519402`／阶段 `a1744de`／房间 `9d5a6af`／
闭环与八条 check `35f3411`；四处偏差已入契约（§0.1）；判据身份与折叠边界已登记
（`77d8dd7`，`docs/verification.md` 新条目）。**本片记为已完成。**

**未跑项（按契约保持未验证，不得当作通过的一部分）**：`-Suite all`／`-UISuite all` 全量回归、
Android 真机、迁移日志的消费方（存档切片暂停中，未消费）。

**跨文件一致性**：`docs/save-fixed-points.md`（暂停中）的挂点已具体化为
`_apply_transition` ＋ `_transition_log`，并写明复工第一步＝"在 `TRANSITIONS` 声明表里标出三个固定点
（kind 级）"，而不是再比 `floor`／`phase`。

**验收建议（规划者）**：**建议按"实现者自测＋协调者复核＋登记"记为完成，不单独派 validator。**
理由：本片判据是"行为逐字节不变"而非新玩法／新界面；**迁移 oracle 的迁移前后摘要对照**
（31 场景、含八类战斗结束入口，跨两次运行稳定）比 UI 人路径更强；UI 侧已由
`-UIOnly -UISuite persistence,home,events`（369 断言）覆盖；协调者已独立重跑主判据。
若协调者仍要派，建议最小范围＝`-UIOnly -UISuite rewards,guard,prison,tower,events -TimeoutSeconds 900`
＋人路径"打赢一场→奖励→整备→下一房"观察阶段与存档一致；**范围由协调者定**。

## 9.1 需人确认（历史：立项时的 `needs-human-review` 理由，已由人审满足）
...
- 来源：人明确指定本片范围——"结束战斗有 8 条路径之类的问题是不可接受的，先把之前改存档时碰到的管线问题收束到同一条主路径上"。
- **计划裁定：接受**（协调者记录；规划者标的四条 `needs-human-review` 理由属本片固有风险，已逐条核实并接受，人可随时叫停）：①核心状态机改动（26＋10＋13 点改道）；②新增两个私有入口、`_finish_battle` 加 `end_kind`、`Guard.capture` 改道；③跨切片影响（存档切片暂停中、固定点判定将挂到本片主路径）；④**判据依赖"收束前先冻结迁移基线"——本条为硬前提**：实现者必须**先**在改动前的提交上抓取并冻结迁移基线（`--write=` 抓、`--baseline=` 比），再开始改道；基线未冻结前不得动 `core/`。
- **排序**：本片先做；`docs/save-fixed-points.md` 保持**暂停**，其 P1／P2／P3 待本片收束后按主路径重写，不得按旧触发集复工；代码半成品在 `stash@{0}`。
- **不新增 `core/*.gd`**（E5 未授权）；若实现中发现必须新增文件，停下交回。
- 已接受的既有红集口径沿用：`{card_power 5, installed_tools 1, tower_progression 10 规则＋1 界面, hand_assist 1}`，`-KeepGoing` 的 `unrun` 必须逐分类补跑。
