...
规划者契约（planner contract）**v2**，2026-09-17。分支 `event-pipeline-unification`，契约撰写时 HEAD `eb85733`；本行不追踪 HEAD，落地时以 `git rev-parse HEAD` 为准（v1 已提交于 `41f37fc`）。
...
**工作区提示**：`spire-godot/tests/check_index.json` 与 `spire-godot/tests/normal_play_cases.gd` 有**他人未提交改动**（2026-09-17：工作区已干净，该提示不再适用）；
...
- 两个消费点各自全建一次：`core/game.gd:dispatch` 的 `for c in candidates():`（**只为找到一行**）
  ...
  实测一次成功提交占一次点击 17.8%／32.7%／41.5%（0／12／26 件；`docs/verification.md` 2026-09-17「卡顿定位」）。
...
### 4.1 组清单（2026-09-17 复核：表内 23 个在用组 ＋ 未使用的默认 `action`；代码侧 `rest` 相位的 `_rest_candidates` 另有 `hook` 组未列本表，组集合待复核）
...
## 6. 既有冲突结论：`response-pipeline.md` §4.1"禁止跨提交缓存候选"（**2026-09-17 已废止，见本节末"后续"段**）
...
**后续（2026-09-17 当日）：人裁定把 §4.1 的三条禁令直接删除，而不是加注。** `docs/response-pipeline.md` §4.1 已写入修订段（准入线改为"复用必须附可证失效规则；无证明即禁止"），本节上面的逐字注与下文的 A6 随之**作废**（不再要求落地第二条注文）。本节保留为分工依据：核心侧"不跨提交保留查询结果"的纪律与"作废集合由核心给出"的判定权仍然成立。
...
- **A6（跨契约注）**：~~§6 的注必须与代码同批落地~~ **已作废（2026-09-17）**：人裁定 §4.1 的禁令直接删除，`response-pipeline` §4.1 已写入修订段（"复用必须附可证失效规则；无证明即禁止"），不再需要第二条注文。§6 的逐字注保留为历史，实现时不得据此要求落地。
