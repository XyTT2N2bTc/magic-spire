...
static func apply_bind(g, enemy: Dictionary) -> void:
 ...
 if has_bind(g,source_kind):
  ...
  return
 var initial=g.IronMan.capture_start(enemy) if enemy.type=="iron_man" else float(g.Enemies.TYPES[enemy.type].capture_start)
...
static func view(g) -> Dictionary:
 ...
 if has_bind(g,"iron_man"): descriptions.append("铁男捕缚：只允许坐姿或躺姿；每累计消耗3能量，触发全部主动刺激型特殊装备。无人机存活时还会施加胶带、捕缚＋5、额外遥控1件装备并随机上锁。当前累计%d/3能量。" % g.state.guard_bind.sources.iron_man.energy)
...
static func validate(g) -> String:
 ...
 for source_kind in bind.sources:
  ...
  var threshold=g.IronMan.ENERGY_THRESHOLD if source_kind=="iron_man" else (2 if source_kind=="drone" else 1)
  ...
  if source_kind not in ["drone","iron_man"] and source.energy!=0: return "此捕缚不累计能量。"
 var allowed=allowed_postures(g)
