...
- **不做文案与投影的按需化**（`card_texts` 按需、候选 `detail` 复用、`escape_preview` 命中率）：
  ...
  该契约目前**不承接 `escape_preview` 的按需化**（其 §0.2／§0.4 把它列为本片/邻接表片范围），
  因此这一子项**当前没有承接片**，归属待协调者裁决（见 §10 实测成本分布、§9）。
...
其中 `escape_preview` 的按需化**两份契约都没有承接**（该契约把它排除在本片之外），归属待协调者裁决。
...
- **真正的成本在别处**（同一批次实测）：文案与投影合计 25.0 + 18.9 = 43.9ms（≈93.2ms 的 47%，
  ...
  `docs/ondemand-copy.md`（先收口文案路由、后按需投影；其中 `escape_preview` 按需化两片都未承接，
  归属待协调者裁决），本片不得顺手做（§0.3／§9 非目标）。
...
- 实现 §0.3 押后、现由 `docs/ondemand-copy.md` 承接的"文案与投影按需化"
  （`card_texts` 按需、候选 `detail` 复用；`escape_preview` 按需化两片都未承接，归属待裁决）——
  在装备片里做即越界；
