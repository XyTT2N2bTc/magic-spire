...
| UI 不读不写 `game.state` | 本片不改 UI；`get_view`／`candidates` 的输出契约不变 |
...
- **不做文案与投影的按需化**（`card_texts` 按需、候选 `detail` 复用、`escape_preview` 命中率）：
  已由人裁定押后为独立一片，本片不得实现（§10 实测成本分布、§9）。
...
已由人裁定**押后为独立一片**，本片不得实现——这恰好是 §10 实测里真正的成本所在。
...
- **真正的成本在别处**（同一批次实测）：文案与投影合计 25.0 + 18.9 = 43.9ms（≈93.2ms 的 47%，
  ...
  该方向（`card_texts` 按需、候选 `detail` 复用、`escape_preview` 命中率）已由人裁定
  **押后为独立一片**，本片不得顺手做（§0.3／§9 非目标）。
...
- 实现 §0.3 押后的"文案与投影按需化"（`card_texts` 按需、候选 `detail` 复用、`escape_preview` 命中率）；
