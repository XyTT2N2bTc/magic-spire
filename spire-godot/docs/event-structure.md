...
> **状态注（2026-09-16，规划者）：本节描述 B1 之前的结构（`choices`／`stages`／`start_stage` 两套形态、
> `content_catalog._flow_references`、`enter_stage`）。现行形态是单一节点形态（`nodes`／`start_node`／
> 节点声明）与单求值入口，见 `docs/event-pipeline-unification.md` 及其"执行记录"。
> §2 缺陷清单中 1／3／4／5／6／8 的当前状态以该契约的执行记录为准：B1 已落地（定义形态归一、`has_relic`
> 校验补 `data`、保留字补 `battle`／`loot`），1／3／4／5 的收敛与 3 的单一声明由 **B2（尚未落地）**
> 承接，8 的"候选期再次探测"在 B2 后仍保留（见该契约 §1.2／§4.4）。本节保留为历史测绘，不重写。**
...
### 7.1 能力普查（内容包实测，2026-09-16）

12 份事件内容：普通 8 份、flow 4 份。选项级功能键的使用：

| 能力 | 普通事件 | flow 事件 | 说明 |
| --- | --- | --- | --- |
| `encounter`（事件战斗） | 1 份 | **不支持** | 多阶段事件插不进战斗 |
| `item_rewards`（道具奖励） | 1 份 | **不支持** | 多阶段事件发不了道具 |
| `hide_when_unavailable` | 5 份 | **不支持** | 多阶段事件的隐藏只能靠 `when` 或冻结失败 |
| `selector`（目标选择） | 2 份 | 3 份 | 重叠区，但 flow 的多个 selection 共享同一次 outcome 抽取 |
| `when`／`next`／`outcomes`／`counter` | **不支持** | 1／4／2／1 份 | 多阶段、条件、跳转、权重随机 |
| `recipe` | 0 份 | 0 份 | 两套实现都在，内容里无人使用 |

效果级统计（全部事件）：`pressure` 16 次、`install_random` 14 次、`card` 7 次、
`counter` 6 次、`special_install_random` 3 次、`relic` 2 次、`tighten_random` 1 次、
`random_amount` 1 次；`hold_special`／`restore_held` **0 次**——flow 专属的"临时收走装备"
没有任何内容使用，而校验器为它维护了 key 唯一与 cleanup 归还配平。

由此，功能差异分三类：

1. **各自独有的能力**（真正的形态差异）：普通＝单步＋战斗＋道具＋按探测隐藏；
   flow＝多阶段＋条件＋跳转＋权重随机。
2. **重叠能力的语义微差**（同一件事两种行为）：`selector` 与 outcome 的抽取次数、
   `recipe` 与 `effects` 的组合方式、效果是否预跑。
3. **两边都实现但无人使用的能力**：`recipe`、`hold_special`／`restore_held`。

归一的价值集中在第 2、3 类；第 1 类应保持各自语义，是否收敛属 E6 的产品决策。

### 7.2 丢弃点与扩展点普查（协调者补注，2026-09-16）

> **状态注（2026-09-16，规划者）：本节行号与"9 处静默丢弃"是本契约落库时的实测。**
> 丢弃点具名化（gate 名）与单求值入口的落地批次、现行定义形态见
> `docs/event-pipeline-unification.md`（§4.2 gate 清单、§9 分批、§16／§17 payload）。
> 本节的"新增一种状态条件要改 3 处"已由该契约 §5 的声明表收敛；行号以落地时仓库为准。
