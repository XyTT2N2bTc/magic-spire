...
static func run(t) -> void:
 ...
 for free in [false,true]:
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"WILDFIRE stale activation preserves payment and physical card")
 ...
 var result=g.dispatch(fire(t,g).id,g.state.version)
