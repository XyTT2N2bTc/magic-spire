...
static func run(t) -> void:
 ...
 t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"OPENER stale skill does not count or damage")
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.powers[0].power_progress==0,"OPENER third skill resets count")
