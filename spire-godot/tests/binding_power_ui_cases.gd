...
static func run(t) -> void:
 ...
 t.check(face.rarity=="common" and face.ILLUSTRATIONS.has("binding_power") and face.get_node("CardCost").text=="1" and text.contains("消耗") and text.contains("拘束1") and text.contains("本回合力量＋2"),"BIND POWER UI displays common art cost exhaust and first bound live gain")
 await Click.click_card(t,card.uid);await t.frames()
 t.check(ui.game.state.turn_strength==2 and ui.game.state.exhaust.any(func(item):return item.uid==card.uid),"BIND POWER UI first face click grants temporary strength and exhausts")
