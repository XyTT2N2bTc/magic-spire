...
static func run(t, equal: Callable) -> void:
 ...
 t.check(g.dispatch(c.id,g.state.version).ok,"SL real attack commits")
 ...
 t.check(not g.dispatch(c.id,0).ok and g.state==before and g.restart_snapshot()==checkpoint,"SL rejected action leaves both live state and checkpoint unchanged")
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g.restart_snapshot()==start,"SL shop checkpoint precedes the first purchase")
...
static func rest_entry(t, equal: Callable) -> void:
 for kind in ["rest_card","rest_rare","rest_flask","rest_begin"]:
  ...
  t.check(g.dispatch(choice.id,g.state.version).ok and g.state.phase=="rest","SL rest enters its playable turn after "+kind)
  ...
  t.check(not g.dispatch(choice.id,start.version).ok and g.state==live and g.restart_snapshot()==start,"SL stale rest choice cannot change either state or entry")
