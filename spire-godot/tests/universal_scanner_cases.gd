...
static func run(t) -> void:
 ...
 g._gain_card("strain")
 var source=g.state.deck.filter(func(card):return card.type=="strain")[0].duplicate(true)
 var room=g.room_data(g.state.room)
 var index=room.stock.find(room.stock.filter(func(row):return row.type==TYPE)[0])
 g.state.mana=64;g.state.flask_mana=65;g.state.temporary_mana=100
 ...
 t.check(purchase.mana==65 and not g.dispatch(purchase.id,g.state.version-1).ok and g.state==before,"SCANNER stale purchase rolls back")
 var count=g.state.deck.size()
 t.check(g.dispatch(purchase.id,g.state.version).ok and g.state.flask_mana==0 and g.state.deck.size()==count and g.state.relic_bundle.source==TYPE,"SCANNER purchase pays once and waits for card choice")
 t.check(g.candidates().all(func(c):return c.payload.kind!="service"),"SCANNER pending selection blocks shop leave, removal and further purchases")
 var choices=g.candidates().filter(func(c):return c.payload.kind=="relic_bundle" and c.payload.op=="copy")
 ...
 t.check(choices.filter(func(c):return c.payload.type=="strain").size()==g.state.deck.filter(func(c):return c.type=="strain").size(),"SCANNER preserves individual choices for same-type physical cards")
 ...
 t.check(restored.dispatch(restored_choice.id,restored.state.version).ok and restored.state.deck.size()==count+1 and restored.state.relic_bundle.is_empty(),"SCANNER restored pending selection can complete once")
 var restored_before=restored.export_snapshot()
 t.check(not restored.dispatch(restored_choice.id,restored.state.version).ok and restored.state==restored_before,"SCANNER restored selection cannot be replayed")
 ...
 t.check(not g.dispatch(choice.id,g.state.version-1).ok and g.state==before,"SCANNER stale card choice cannot create a copy")
 t.check(g.dispatch(choice.id,g.state.version).ok and g.state.relic_bundle.is_empty() and g.state.deck.size()==count+1,"SCANNER selected card is copied exactly once and returns to shop")
 ...
 t.check(g.state.mana==64 and g.state.flask_mana==0 and g.state.temporary_mana==100 and g.validate()=="","SCANNER selection costs no further resources and keeps valid state")
 before=g.export_snapshot()
 t.check(not g.dispatch(choice.id,g.state.version).ok and g.state==before,"SCANNER replayed consumed choice cannot duplicate again")
 ...
 t.check(book.group=="商店限定" and book.rarity=="uncommon" and book.text.contains("双面唯一"),"SCANNER catalog presents source rarity and exclusion")
 ...
 t.check(t.action(witch,"relic_bundle",{"op":"copy","uid":training.uid}).ok and witch.state.deck.back().practice_plays==12 and witch.state.deck.back().type==training.type and witch.state.deck.back().uid!=training.uid and witch.validate()=="","SCANNER copies permanent training progress for character two without sharing identity")
