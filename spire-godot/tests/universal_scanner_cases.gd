...
static func run(t) -> void:
 ...
 t.check(purchase.mana==70 and not g.dispatch(purchase.id,g.state.version-1).ok and g.state==before,"SCANNER stale purchase rolls back")
 ...
 t.check(choices.all(func(c):return rules.SPECS[c.payload.type].rarity!="basic"),"SCANNER excludes basic cards from copy candidates")
