...
static func run(t) -> void:
 ...
 for item in [["strike",0,8.0,1,1],["strike",1,4.0,2,1],["heavy",0,18.0,1,2],["heavy",1,6.0,3,2]]:
  ...
  t.check(g.dispatch(c.id,g.state.version).ok and g._enemy(id).hp==hp-item[2]*item[3] and g.state.energy==energy-item[4],"BASIC multi-hit pays once and applies each hit")
 ...
 for i in range(2):
  ...
  t.check(g.dispatch(sweep.id,g.state.version).ok,"BASIC sweep can repeat with enough energy")
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.enemies[0].gone and g.state.enemies[1].hp==other_hp and g.state.logs.slice(start).filter(func(log):return log.data.has("hit")).size()==1,"BASIC early kill stops remaining hits without retargeting")
 var before=g.export_snapshot()
 t.check(not g.dispatch(c.id,before.version-1).ok and g.state==before,"BASIC stale form submission rolls back")
...
static func zero_energy_fireball(t) -> void:
 ...
 t.check(not c.valid and c.cost==1 and not g.dispatch(c.id,g.state.version).ok and g.state==before,"FIREBALL first use requires one energy and rejected previews cannot consume it")
 g.state.energy=1
 for used in range(2):
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"FIREBALL stale submission cannot consume first-use record")
  t.check(g.dispatch(c.id,g.state.version).ok and g.state.energy==0 and g.state.mana==before.mana-10 and g._enemy(target.id).hp==before.enemies[0].hp-c.payload.damage and g.BasicAttacks.usage(g,"fireball").used==used+1,"FIREBALL correct payment, damage and shared use count")
 c=attack(t,g,"fireball",0);before=g.export_snapshot()
 t.check(not c.valid and c.reason.contains("次数") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"FIREBALL zero later cost cannot bypass turn limit")
 ...
 c=attack(t,g,"fireball",0);before=g.export_snapshot()
 t.check(not c.valid and c.reason.contains("魔力") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"FIREBALL insufficient mana does not consume first use")
 g=Game.new(42)
 ...
 t.check(c.valid and c.cost==1 and g.dispatch(c.id,g.state.version).ok and g.state.energy==0 and g.state.mana==before.mana-c.mana and g._equipment(restraint.id).durability==60.0-c.payload.damage and attack(t,g,"fireball",0).cost==0,"FIREBALL equipment first cast pays once and shares enemy first-use state")
 ...
 for posture in ["stand","sit","lie"]:
  for severity in range(restraints.size()):
   ...
   if allowed:
    ...
    t.check(g.dispatch(attack(t,g,"kick",2).id,g.state.version).ok,"KICK ordinary form can repeat in the same turn")
   else:
    t.check(c.reason!="" and not g.dispatch(c.id,g.state.version).ok and g.state==before,"KICK unavailable stance or severity rejects without resource changes")
...
static func interrupt_cooldown(t) -> void:
 ...
 for i in range(3):
  ...
  t.check(not g.dispatch(kick.id,g.state.version-1).ok and g.state==before,"JUSTICE stale attack is atomic")
  t.check(g.dispatch(kick.id,g.state.version).ok and g.state.energy==before.energy-2 and g.state.enemies[0].hp==before.enemies[0].hp-18 and not g.state.enemies[0].intent.delayed and g.state.kick_last==1,"JUSTICE repeats in first round without reading or extending shared cooldown")
 g.state.energy=1;var before=g.export_snapshot()
 t.check(not g.dispatch(attack(t,g,"kick",0).id,g.state.version).ok and g.state==before,"JUSTICE still requires its actual energy cost")
...
static func justice_opening(t) -> void:
 ...
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before,"JUSTICE restricted legs cannot bypass the candidate")
...
static func bound_kick_cooldown(t) -> void:
 for posture in ["stand","sit"]:
  ...
  for elapsed in range(3):
   ...
   t.check(not c.valid and c.reason.contains("冷却") and g.BasicAttacks.kick_cooldown(g)==3-elapsed and not g.dispatch(c.id,g.state.version).ok and g.state==before,"BOUND KICK sitting cannot bypass cooldown or consume resources")
 ...
 var c=attack(t,g,"kick",0);var before=g.export_snapshot()
 t.check(not c.valid and c.reason.contains("无法在踢击后躺下") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"BOUND KICK cannot waive mandatory fall when capture fixes the sitting posture")
