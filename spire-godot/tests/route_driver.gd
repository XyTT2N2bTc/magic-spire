...
static func event_action(candidates: Array) -> Dictionary:
 var choices=candidates.filter(func(c):return c.payload.kind=="event" and c.valid)
...
static func prepare_action(g) -> Dictionary:
 ...
 var choices=g.candidates().filter(func(c):return c.valid and c.payload.get("target","") in g.equipment_at("mouth").map(func(e):return e.id))
 for c in choices:
