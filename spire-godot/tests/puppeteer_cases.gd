...
static func barrier_and_stock(t) -> void:
 for amount in [29.0,30.0,31.0]:
  ...
  t.check(master.hp==96-minf(amount,30) and g.Enemies.barrier_remaining(master)==maxf(0,30-amount),"PUPPET BARRIER boundary clamps actual turn damage")
 var g=start(t,false);var master=g.state.enemies[0]
 ...
 t.check(twin.Enemies.barrier_remaining(twin.state.enemies[0])==0,"PUPPET BARRIER restore preserves exhausted allowance")
