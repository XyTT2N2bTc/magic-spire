...
static func run(t) -> void:
 ...
 t.check(not g.dispatch(c.id,g.state.version-1).ok and before==g.state,"TEMP stale payment leaves both pools unchanged")
 ...
 t.check(not c.valid and c.reason.contains("2.5") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"TEMP insufficient combined balance rejects atomically")
 g.state.mana=2.5;c=fire(t,g)
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.mana==0 and g.state.temporary_mana==0 and g.state.combat.mana_used,"TEMP fractional remainder pays exact remaining permanent mana")
 g=Game.new(42);g.state.temporary_mana=15;g.state.mana=50
 t.check(g.dispatch(fire(t,g).id,g.state.version).ok and g.state.temporary_mana==5 and g.state.mana==50,"TEMP unused point balance remains after first spell")
 ...
 result=g.dispatch(fire(t,g).id,g.state.version)
 ...
 t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.state==before,"TEMP cannot deposit independent pool")
...
static func ease_preparation(t) -> void:
 ...
 t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"EASE stale preparation leaves all state unchanged")
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.temporary_mana==27.5 and g.state.mana==100 and g.state.energy==before.energy-1 and g.state.rng==before.rng,"EASE two stacks add ten uncapped temporary points without a mouth roll")
