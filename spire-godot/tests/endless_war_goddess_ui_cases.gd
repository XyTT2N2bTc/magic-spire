...
static func run(t) -> void:
 ...
 t.check(face.rarity=="rare" and t.visible_text(face).contains("无尽魔法少女战神") and face.get_node("CardIllustration").texture!=null,"WAR GODDESS UI shows rare card and dedicated vector illustration")
 ...
 for form in range(7):
  ...
  var choice=ui.actions.find("attack",{"type":"kick","form":form,"enemy":ui.selected_enemy})
 ...
 t.check(ui.attack_forms.kick==0 and ui.game.export_snapshot()==before,"WAR GODDESS UI seven-form cycle is read only")
