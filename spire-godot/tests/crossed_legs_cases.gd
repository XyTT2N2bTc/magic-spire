...
static func run(t) -> void:
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.hand.size()==1 and g.state.draw.size()==before.draw.size()-1 and is_equal_approx(g._equipment(target.id).durability,60-c.payload.preview.damage),"CROSS bound face applies ordinary slip formula then draws one")
 ...
 for restricted in [false,true]:
  ...
  if restricted:
   t.check(c.reason.contains("腿部束缚等级≤1") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"CROSS blocked free face explains actual leg level and preserves all state")
  else:
   t.check(g.dispatch(c.id,g.state.version).ok and g.state.energy==0 and g.state.mana==0 and g.state.hand.size()==1 and g.state.draw.size()==before.draw.size()-1,"CROSS level one free face draws at zero energy without refund or mana")
 ...
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before,"CROSS commit rechecks changed leg restriction before drawing")
