...
static func run(t) -> void:
 ...
 for free in [false,true]:
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"READY stale selection rejects before costs or exhaustion")
 ...
 t.check(not c.valid and c.reason.contains("另一张") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"READY cannot consume itself when it is the only hand card")
 ...
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before,"READY target no longer in hand refuses atomically even without version change")
 for free in [false,true]:
...
static func physical_discount(t) -> void:
 ...
 t.check(not duplicate.valid and not g.dispatch(duplicate.id,g.state.version).ok and g.state==before,"READY existing next-attack effect rejects stacking before exhausting another card")
 var fire=t.find_action(g,"attack",{"type":"fireball"})
 t.check(fire.cost==1 and g.dispatch(fire.id,g.state.version).ok and "ready_to_strike_free" in g.state.card_buffs,"READY first fireball keeps its one-energy cost and does not consume physical discount")
 ...
 t.check(kick.valid and kick.cost==0 and g.dispatch(kick.id,g.state.version).ok and g.state.energy==0,"READY seated kick works at zero energy without negative cost")
