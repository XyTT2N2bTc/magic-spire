...
static func run(t) -> void:
 for free in [false,true]:
  ...
  t.check(not g.dispatch(c.id,g.state.version).ok and g.state==after,"SIPHON STRENGTH repeated submission cannot pay or exhaust twice")
 for free in [false,true]:
  ...
  t.check(g.dispatch(c.id,g.state.version).ok and g._magic_failed and g.state.energy==before.energy-1,"SIPHON STRENGTH both faces use normal hand pressure casting failure")
