...
static func run(t) -> void:
 ...
 for tier in g.RelicRewards.TIERS:
  ...
  t.check(offer_tier(g,tier)=="rolling_log" and others.all(func(id):return id not in g.state.relic_seen),"LOG exhausted tier returns a log without consuming another tier: "+tier)
 ...
 for i in range(3):
  t.check(g.RelicRewards.offer(g)=="rolling_log","LOG full exhaustion can repeatedly offer fallback")
 ...
 t.check(g.state.battle_relic_drop=="rolling_log","LOG elite exhaustion freezes fallback reward")
 var pick=t.find_action(g,"reward",{"category":"relic"});var version=g.state.version
 t.check(pick.valid and g.dispatch(pick.id,version).ok and g.state.relic_counters.rolling_log==4,"LOG existing owner can claim another elite copy")
 ...
 for kind in ["shop","treasure"]:
  ...
  t.check(logs.size()==(3 if kind=="shop" else 1) and logs.all(func(offer):return offer.type=="rolling_log"),"LOG fills exhausted "+kind+" relic slots")
