...
static func run(t) -> void:
 ...
 for free in [false,true]:
  ...
  t.check(g.state==before and not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"SUPPLE queries and stale requests grant no attributes")
  t.check(g.dispatch(c.id,g.state.version).ok and g.RelicEffects.attribute(g,"strength")== (2 if free else 0) and g.RelicEffects.attribute(g,"dexterity")== (0 if free else 4) and g.state.energy==0 and g.state.mana==before.mana and g.state.rng.magic==before.rng.magic and g.state.discard.any(func(item):return item.uid==card.uid),"SUPPLE zero-energy play grants the correct temporary attribute and discards")
 ...
 t.check(strike.payload.damage==14 and g.dispatch(strike.id,g.state.version).ok and g._enemy(enemy_id).hp==old_hp-14 and g.Cards.buff_stacks(g,"supple_flesh_free")==1,"SUPPLE strength reaches real martial preview and damage without consuming the turn buff")
 for enemy in g.state.enemies: enemy.intent.delayed=true
 ...
 t.check(slip.valid and damage>before and g.dispatch(slip.id,g.state.version).ok and is_equal_approx(g._equipment(target.id).durability,16-damage) and g.RelicEffects.attribute(g,"dexterity")==4,"SUPPLE dexterity increases real slip preview and damage without consuming stacks: "+str([slip.valid,slip.reason,before,damage]))
