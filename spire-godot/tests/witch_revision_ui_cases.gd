...
static func run(t) -> void:
 ...
 var card=Give.give(ui.game,"witch_escape_practice");card.practice_plays=9
 for permanent in ui.game.state.deck:
  if permanent.uid==card.uid: permanent.practice_plays=9
 ...
 t.check(ui.term_popup!=null and hover_text.contains("9／10次") and hover_text.contains("再使用1次升级"),"WITCH REV UI mouse hover always shows training instance upgrade progress: "+hover_text)
 await t.flip(card.uid);await t.move_mouse(t.card_point(card.uid));await t.frames()
 t.check(t.visible_text(ui.term_popup).contains("9／10次"),"WITCH REV UI both bound faces share hover progress: "+t.visible_text(ui.term_popup))
 ...
 t.check(ui.localization.display("升级进度：9／10次；再使用1次升级。").contains("9/10") and ui.localization.display("拘束诱导")=="Binding Lure","WITCH REV UI translated progress and new card name")
