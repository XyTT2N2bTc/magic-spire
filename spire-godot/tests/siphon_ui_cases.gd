...
static func run(t) -> void:
 ...
 t.check(t.visible_text(face).contains("汲取") and face.rarity=="common" and t.visible_text(face.get_node("CardMana/Mana_gain")).strip_edges()=="+5" and ui.actions.find("card",{"uid":card.uid,"free":false}).cost==0,"SIPHON UI bound face displays name rarity zero energy and five mana")
 ...
 t.check(t.visible_text(face).contains("抽牌2") and t.visible_text(face.get_node("CardMana/Mana_gain")).strip_edges()=="+10" and ui.actions.find("card",{"uid":card.uid,"free":true}).cost==1,"SIPHON UI free face shows one energy ten mana and draw two")
