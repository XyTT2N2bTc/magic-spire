...
static func continuation_recovery(t) -> void:
 for posture in ["stand","sit","lie"]:
  for pressure in [0.0,25.0,40.0,80.0]:
   ...
   t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"DEMO rejected continuation preserves pressure and posture")
   t.check(g.dispatch(c.id,g.state.version).ok and g.state.pressure==maxf(0.0,pressure-40.0) and g.state.posture=="stand","DEMO continuing from every posture reduces pressure once with a zero floor")
