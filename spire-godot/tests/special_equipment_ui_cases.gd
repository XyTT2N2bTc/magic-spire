...
static func run(t) -> void:
 ...
 t.check(t.visible_text(details).contains("每次高潮") and t.visible_text(details).contains("6－当前紧度档位－装备等级") and t.visible_text(details).contains("当前2点"),"SPECIAL UI urethral rod detail shows formula and current fixed slip damage")
 ...
 t.check(t.visible_text(ui.find_child("EquipmentDetails",true,false)).contains("电量剩余3回合"),"SPECIAL UI displays the real remaining battery after two player turns")
 await t.close_information()
 ...
 t.check(t.visible_text(details).contains("高级平板锁加固带") and text.contains("负数跳蛋锁（导尿管）") and text.contains("自动上锁") and text.contains("0.4"),"CHASTITY UI explains the lock, linked reinforcement, automatic lock and vibrator factor")
