...
static func run(t) -> void:
 ...
 t.check(not Cards.play(t,g,"echo_cast",true).ok,"ECHO same pending face cannot stack")
 ...
 t.check(is_equal_approx(g.state.enemies[0].hp,before.enemies[0].hp-2*c.payload.damage) and g.state.enemies[1].hp==before.enemies[1].hp and "echo_cast_free" not in g.state.card_buffs,"ECHO double damage to only original target and consumes buff")
 ...
 # On-use powers see both actual casts; first-only base bonus is not copied.
 ...
 t.check(result.ok and g.state.hand.size()==before.hand.size()+2 and is_equal_approx(g.state.enemies[0].hp,before.enemies[0].hp-c.payload.damage-g.BasicAttacks.fireball_damage(g)),"ECHO both actual casts draw, second recomputes spent embers bonus")
 ...
 for type in ["mana_invocation","mana_surge","rekindle","wildfire_descent","pleasure_conversion"]:
  t.check(not g.Cards.Rules.distinct_faces(type),"ECHO equal mechanical faces excluded: "+type)
 t.check(Cards.play(t,g,"mana_surge",false).ok and g.state.charge==2 and "echo_cast_bound" in g.state.card_buffs,"ECHO same-face spell neither repeats nor consumes")
...
static func boundaries(t) -> void:
 ...
 t.check(result.ok and casts.size()==2 and casts[0].data.spell.roll!=casts[1].data.spell.roll and g.state.hand.size()==before.hand.size()+2 and g.state.mana==before.mana-c.mana,"ECHO two independent cast rolls, draw on both attempts, one payment")
