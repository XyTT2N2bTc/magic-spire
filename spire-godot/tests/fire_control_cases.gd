...
static func run(t) -> void:
 ...
 t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"CONTROL stale play rejects without permanent gain")
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.energy==2 and g.state.spell_base_bonuses.fireball==2 and g.state.exhaust.any(func(e):return e.uid==card.uid),"CONTROL free face adds permanent damage and exhausts physical card")
 t.check(fire(t,g).payload.damage==g.B.FIREBALL_ASSISTED+2,"CONTROL gesture fireball base gains two")
 card=Cards.give(g,"fire_control")
 t.check(t.action(g,"card",{"uid":card.uid,"free":true}).ok and g.state.spell_base_bonuses.fireball==4,"CONTROL separate copies stack permanent base")
 g.state.card_buffs=["henshin_free"]
 t.check(fire(t,g).payload.damage==(g.B.FIREBALL_ASSISTED+4)*2,"CONTROL adds before global damage multiplier")
 ...
 t.check(fire(t,g).payload.damage==g.B.FIREBALL+4,"CONTROL no-gesture fireball gains same permanent amount")
 card=Cards.give(g,"fire_control");c=t.find_action(g,"card",{"uid":card.uid,"free":true},false)
 before=g.export_snapshot()
 t.check(not c.valid and c.reason.contains("手掌") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"CONTROL one hand with low tightness blocks free face atomically")
 ...
 t.check(t.action(g,"card",{"uid":card.uid,"free":false}).ok and g.state.temporary_mana==reserve+10 and g.state.rng.magic==rng and g.state.spell_base_bonuses.fireball==4,"CONTROL bound face grants two reserves without mana or spell roll")
 g.RelicEffects.end_combat(g)
 t.check(g.state.spell_base_bonuses.fireball==4,"CONTROL permanent gain survives scene cleanup")
 g.state.phase="battle";g.RelicEffects.begin_combat(g)
 t.check(g.state.spell_base_bonuses.fireball==4 and g.validate()=="","CONTROL next battle preserves permanent gain")
 var twin=Game.new(42)
 t.check(twin.restore_snapshot(g.export_snapshot()).ok and twin.state.spell_base_bonuses.fireball==4,"CONTROL current-format snapshot preserves permanent gain")
...
static func revised_conditions(t) -> void:
 ...
 t.check(g.restraint_degree("arms")==1 and c.valid and g.dispatch(c.id,g.state.version).ok and g.state.spell_base_bonuses.fireball==2 and g.state.rng.magic==before.rng.magic and g.state.mana==before.mana,"CONTROL upper total exactly one permits permanent two without a spell roll at high pressure")
 ...
 t.check(t.action(g,"card",{"uid":card.uid,"free":true}).ok and g.state.spell_base_bonuses.fireball==2,"CONTROL existing casting manual allows one completely free hand")
