...
static func run(t) -> void:
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.energy==2 and g.state.spell_base_bonuses.fireball==1 and g.state.exhaust.any(func(e):return e.uid==card.uid),"CONTROL free face adds permanent damage and exhausts physical card")
...
static func revised_conditions(t) -> void:
 ...
 t.check(g.restraint_degree("arms")==1 and c.valid and g.dispatch(c.id,g.state.version).ok and g.state.spell_base_bonuses.fireball==1 and g.state.rng.magic==before.rng.magic and g.state.mana==before.mana,"CONTROL upper total exactly one permits permanent one without a spell roll at high pressure")
