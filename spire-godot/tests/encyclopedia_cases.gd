...
static func run(t) -> void:
 ...
 for entry in items:
  ...
  t.check(entry.text.contains(Book.Tools.description(neutral,entry.id)) and entry.text.contains("使用次数：%d" % spec.uses),"BOOK item effect and charges share inventory formatter "+entry.id)
