...
static func run(t) -> void:
 ...
 var candidates=g.candidates().filter(func(c):return c.payload.kind=="item_use" and c.payload.item==id)
 t.check(candidates.size()==14 and candidates.all(func(c):return c.valid),"OIL all panel groups selectable while standing with hands and mouth bound")
