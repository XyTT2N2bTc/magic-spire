...
static func run(t) -> void:
 ...
 for x in [1,2,3]:
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.export_snapshot()==before,"SELF BIND stale submission is atomic")
  t.check(g.dispatch(c.id,g.state.version).ok,"SELF BIND free executes through dispatch")
 ...
 for x in [0,4]:
  ...
  var c=t.find_action(g,"card",{"uid":card.uid,"free":true})
  t.check(not c.valid and c.reason.contains("品质＋紧度") and not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SELF BIND impossible grade sum refuses without effects")
 ...
 var before=g.export_snapshot()
 var c=t.find_action(g,"card",{"uid":card.uid,"free":true})
 t.check(not c.valid and c.reason.contains("足够位置") and not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SELF BIND only one slot refuses the whole two-item cost and refund")
 ...
 t.check(c.valid and g.dispatch(c.id,g.state.version).ok and g.state.equipment.size()==old_ids.size()+2 and old_ids.all(func(id):return not g._equipment(id).is_empty()),"SELF BIND exact two-slot boundary succeeds without replacing existing equipment")
 ...
 for x in [0,1,4]:
  ...
  t.check(c.valid and g.dispatch(c.id,g.state.version).ok,"SELF BIND complete bound capacity succeeds for X="+str(x))
 ...
 before=g.export_snapshot();c=t.find_action(g,"card",{"uid":card.uid,"free":false})
 t.check(not c.valid and c.reason.contains("需要收紧4档") and c.reason.contains("只能收紧2档") and not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SELF BIND insufficient tightening never partially tightens or grants mana")
 ...
 t.check(not c.valid and c.reason=="休息房禁止卡牌自由效果。" and not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SELF BIND original rest-room free-face restriction still takes priority")
