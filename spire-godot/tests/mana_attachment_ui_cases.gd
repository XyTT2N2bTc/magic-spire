...
static func run(t) -> void:
 ...
 var attack=ui.actions.find("attack",{"type":"strike"})
 t.check(attack.payload.all and attack.mana==10,"ATTACHMENT UI attack candidates show paid AOE immediately")
 ...
 attack=ui.actions.find("attack",{"type":"strike"})
 ...
 await t.mouse_button(point,MOUSE_BUTTON_RIGHT,true);await t.mouse_button(point,MOUSE_BUTTON_RIGHT,false)
 t.check(ui.actions.find("attack",{"type":"strike"}).payload.all and ui.game.state.mana==mana,"ATTACHMENT UI second right click resumes AOE without payment")
