...
static func run(t) -> void:
 ...
 for i in range(30): ui.game._gain_card("slip")
 ...
 t.check(ui.view.reward_panel.get("selection","")=="card_copy" and ui.view.mana==35 and ui.game.state.deck.size()==count,"SCANNER UI paid pickup opens choice without granting a random card")
 var browser=ui.find_child("DeckBrowser",true,false)
 t.check(browser!=null and browser.cards.all(func(card):return card.type!="endless_war_goddess"),"SCANNER UI browses eligible physical cards and excludes double-unique power")
 var before=ui.game.export_snapshot()
 var search=browser.find_child("DeckSearch",true,false);search.text="用力";search.text_changed.emit(search.text);await t.frames()
 ...
 t.check(ui.view.mana==35 and ui.find_child("RelicShortcut_"+Cases.TYPE,true,false)!=null,"SCANNER UI retains pickup icon and does not charge for selection")
 ui.restart(42);ui.game=Cases.shop()
