...
static func run(t) -> void:
 ...
 t.check(quoted.price==10 and quoted.price_detail.contains("折后10魔力") and release.mana==10 and g.dispatch(release.id,g.state.version).ok and g.state.flask_mana==balance-10,"MEMBERSHIP release quote and real payment receive exactly one half discount")
 g.state.shop_removals=1
 var removal=t.find_action(g,"service",{"op":"remove","payment":"flask"})
 balance=g.state.flask_mana
 t.check(g.get_view().shop.remove_price==25 and removal.mana==25 and g.dispatch(removal.id,g.state.version).ok and g.state.flask_mana==balance-25 and g.state.shop_removals==2,"MEMBERSHIP removal discount applies after its persistent base-price increase")
