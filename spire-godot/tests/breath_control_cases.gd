...
static func run(t) -> void:
 for free in [false,true]:
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"BREATH stale choice rejects both targets atomically")
 ...
 for tightness in [1.0,4.0,8.0,10.0]:
  ...
  t.check(not c.valid and c.reason.contains("口部无拘束") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"BREATH every positive mouth tier blocks free face without spending or exhausting")
 ...
 t.check(not c.valid and c.reason.contains("另一张") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"BREATH source cannot exhaust itself when alone")
 ...
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before,"BREATH removed selected card rejects without charge, discount or payment")
 ...
 t.check(c.cost==0 and not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"BREATH discounted attack still revalidates version")
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.energy==1 and g.state.charge==0 and g._enemy(c.payload.enemy).hp<hp and "breath_control_free" not in g.state.card_buffs and t.find_action(g,"attack",{"type":"strike","form":0}).cost==1,"BREATH next physical attack consumes charge and discount once, following cost restored")
