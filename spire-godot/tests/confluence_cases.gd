...
static func run(t) -> void:
 ...
 for count in range(5):
  for free in [false,true]:
   ...
   t.check(text.contains("每佩戴2件拘束具，本回合获得1点力量，不足2件不计。") if not free else text.contains("每佩戴1件拘束具，恢复1点魔力。"),"CONFLUENCE card always explains the rule, including zero equipment")
   t.check(text.contains("当前：本回合力量＋%d。" % int(count/2)) if not free else text.contains("当前：恢复%d魔力。" % count),"CONFLUENCE current result supplements the permanent rule")
   t.check(c.valid and c.cost==0 and c.mana==0 and not c.get("casting",{}).get("roll_required",false) and not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"CONFLUENCE no casting or energy and stale request rolls back")
   t.check(g.dispatch(c.id,g.state.version).ok and g.state.strength==0 and g.state.turn_strength==(int(count/2) if not free else 0) and g.state.mana==40+(0 if not free else count) and g.state.energy==before.energy and g.state.discard.any(func(x):return x.uid==card.uid),"CONFLUENCE odd even zero counts grant exact values and discard physical card")
 ...
 t.check(g.Cards.worn_count(g)==3 and Give.play(t,g,"confluence",true).ok and g.state.mana==43,"CONFLUENCE composite and special count once")
 ...
 t.check(g.state.turn_strength==2 and g.Cards.worn_count(g)==1 and g.Cards.metadata(g,"confluence").face_mana.free[0].text=="+1" and g.Cards.face_text(g,"confluence",false).contains("力量＋0"),"CONFLUENCE equipment change updates next card values without revoking old strength")
 ...
 t.check(descriptions[1].contains("每佩戴2件") and descriptions[2].contains("每佩戴1件") and not descriptions[1].contains("当前：") and not descriptions[2].contains("当前："),"CONFLUENCE context-free catalog keeps both formulas without inventing a current value")
