...
static func run(t) -> void:
 ...
 for count in range(5):
  for free in [false,true]:
   ...
   t.check(g.dispatch(c.id,g.state.version).ok and g.state.strength==0 and g.state.turn_strength==(int(count/2) if not free else 0) and g.state.mana==40+(0 if not free else count*2) and g.state.energy==before.energy,"CONFLUENCE odd even zero counts grant exact values")
