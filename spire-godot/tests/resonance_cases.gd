...
static func run(t) -> void:
 ...
 t.check(c.valid and c.cost==1 and c.mana==0 and not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"RESONANCE stale activation is atomic and costs only one energy")
 t.check(g.dispatch(c.id,g.state.version).ok and g._mana_cost(10)==10,"RESONANCE no equipment means no discount")
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g._magic_failed and is_equal_approx(g.state.mana,before.mana-4.75),"RESONANCE failed cast refunds half the discounted nine point five cost")
