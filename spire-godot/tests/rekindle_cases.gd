...
static func run(t) -> void:
 ...
 for free in [false,true]:
  ...
  var before=g.export_snapshot()
  t.check(not c.valid and c.reason.contains("手") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"REKINDLE hand requirement blocks either face without payment")
