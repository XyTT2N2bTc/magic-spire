...
static func run(t) -> void:
 ...
 t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.state==before,"WAND zero points cannot create a free action")
 for n in range(14):
 ...
 t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"WAND stale exchange leaves counter and mana intact")
