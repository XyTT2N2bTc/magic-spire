...
static func run(t) -> void:
 ...
 t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"LEVERAGE stale target play has no effects")
 t.check(g.dispatch(c.id,g.state.version).ok and is_equal_approx(g._equipment(target.id).durability,80-c.payload.preview.damage) and is_equal_approx(g._equipment(peer.id).durability,60-splash[0].preview.damage),"LEVERAGE real damage and collateral match their independent multipliers")
...
static func free_face(t) -> void:
 ...
 var before=g.export_snapshot()
 t.check(c.valid and g.dispatch(c.id,g.state.version).ok and g.state.evasion==1 and g.state.charge==1,"LEVERAGE free face grants evasion and charge despite leg restraints")
 ...
 t.check(not c.valid and c.reason.contains("上身") and not g.dispatch(c.id,g.state.version).ok and g.state==before,"LEVERAGE upper restriction rejects free face atomically with a concrete reason")
 blocked.durability=0;g._cleanup();g.state.energy=0
 c=t.find_action(g,"card",{"uid":card.uid,"free":true});before=g.export_snapshot()
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before,"LEVERAGE insufficient energy cannot grant either buff")
 g.state.energy=2
