...
static func run(t) -> void:
 ...
 var candidate=ui.actions.find("card",{"uid":uid,"target":target,"free":false})
 ...
 t.check(ui.drop_targets.has(candidate.id),"BIND UI connection has its own formal drop target")
