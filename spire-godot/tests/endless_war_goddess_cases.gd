...
static func original_attacks(t,g) -> void:
 ...
 t.check(choices.size()==12 and choices.all(func(c):return c.valid) and g.state==before,"WAR GODDESS all original forms remain usable lying and heavily restrained; preview is read only")
 var kicks=choices.filter(func(c):return c.payload.type=="kick")
 t.check(kicks.map(func(c):return c.label)==["正义飞踢","横扫","站着踢","并拢飞踢","并腿蹬击","坐姿踢击","坐着踢"],"WAR GODDESS right-click candidates include every posture and bound kick form")
 ...
 t.check(t.action(g,"attack",{"type":"kick","form":3}).ok and not t.find_action(g,"attack",{"type":"kick","form":4},false).valid,"WAR GODDESS bound forms retain shared kick cooldown")
 ...
 g._finish_battle()
 t.check(not g.Cards.basic_attack_freedom(g) and g.BasicAttacks.forms(g,"kick").size()==3,"WAR GODDESS freedom ends immediately when combat finishes")
