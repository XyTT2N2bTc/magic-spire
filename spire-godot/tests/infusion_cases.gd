...
static func run(t) -> void:
 for free in [false,true]:
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"INFUSION stale cast is atomic")
  t.check(g.dispatch(c.id,g.state.version).ok and buff in g.state.card_buffs and g.state.energy==before.energy-c.cost and g.state.mana==before.mana-c.mana and g.state.discard.any(func(x):return x.uid==card.uid),"INFUSION successful cast pays once grants its pending effect and discards normally")
 ...
 for free in [false,true]:
  for type in ["strike","heavy","kick","fireball"]:
   ...
   var buff="infusion_free" if free else "infusion_bound"
   t.check(g.dispatch(c.id,g.state.version).ok and (buff in g.state.card_buffs)==not matches and g.state.enemies[0].intent.delayed==matches,"INFUSION only the next matching physical attack consumes and interrupts: "+str(free)+type)
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.enemies[0].hp==before.enemies[0].hp-18 and g.state.card_buffs.is_empty(),"INFUSION multi-hit short strike uses both matching preparations once")
 ...
 t.check(c.payload.interrupt and c.payload.fall and g.dispatch(c.id,g.state.version).ok and "infusion_bound" not in g.state.card_buffs and g.state.logs.slice(before.logs.size()).filter(func(log):return log.data.has("interrupt")).size()==1,"INFUSION innate interrupt consumes the preparation but never doubles delay")
 g=setup();t.check(cast(t,g,false).ok,"INFUSION prepare area attack")
 c=attack(t,g,"kick",1)
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.enemies.all(func(e):return e.intent.delayed) and "infusion_bound" not in g.state.card_buffs,"INFUSION sweep interrupts each living target and consumes once")
 g=setup();cast(t,g,true);g.state.energy=0;c=attack(t,g,"strike");before=g.export_snapshot()
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before,"INFUSION invalid physical attack preserves the preparation")
 g=setup();cast(t,g,true);g.state.enemies[0].hp=1;c=attack(t,g,"strike");before=g.export_snapshot()
 t.check(g.dispatch(c.id,g.state.version).ok and "infusion_free" not in g.state.card_buffs and g.state.logs.slice(before.logs.size()).all(func(log):return not log.data.has("interrupt")),"INFUSION lethal attack consumes its preparation without delaying a defeated enemy")
