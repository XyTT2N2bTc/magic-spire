...
static func run(t) -> void:
 ...
 for seed_value in range(8):
  ...
  t.check(g.state.energy==before.energy-chosen.cost and is_equal_approx(g.state.mana,before.mana-chosen.mana) and (g.state.discard if result.success else g.state.hand).any(func(c):return c.uid==card.uid),"CAST both outcomes pay, only success discards the card")
 ...
 card=t.hand_card(g,"unlock");before=g.export_snapshot()
