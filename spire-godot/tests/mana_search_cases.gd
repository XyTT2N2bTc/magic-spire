...
static func run(t) -> void:
 ...
 t.check(g.state==before and not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"SEARCH query and stale command are atomic")
 ...
 for slot in ["eyes","mouth","upper_arm","forearm","wrist","palm","fingers","thigh"]:
  ...
  if not allowed:
   ...
   t.check(not g.dispatch(c.id,g.state.version).ok and g.state==before and c.reason.contains("束缚等级≤0"),"SEARCH blocked free face preserves all resources and piles")
 ...
 t.check(g.level("arms")==1 and not g.dispatch(c.id,g.state.version).ok and g.state==before,"SEARCH commit rechecks total level and rejects atomically")
