...
static func run(t) -> void:
 ...
 var candidate=ui.actions.find("card",{"uid":uid,"target":left.id,"free":false})
 ...
 t.check(ui.drop_targets.has(candidate.id),"SHOULDER UI drag opens actual shoulder candidate")
