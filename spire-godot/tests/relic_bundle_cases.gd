...
static func run(t) -> void:
 ...
 var frozen=g.export_snapshot();var view=g.get_view();g.get_view();g.candidates()
 t.check(g.state==frozen and view.reward_panel.layout=="relic_bundle" and view.reward_panel.entries.size()==3,"BUNDLE read-only views and candidates neither reroll nor grant rewards")
 var c=t.find_action(g,"relic_bundle",{"op":"claim","index":0})
 t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==frozen,"BUNDLE stale child claim rejects atomically")
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and entries[0].type in g.state.relics and g.state.relic_bundle.entries[0].status=="claimed","BUNDLE first child only granted on claim")
 var claimed=g.export_snapshot()
 t.check(not g.dispatch(c.id,g.state.version).ok and g.state==claimed,"BUNDLE cannot claim the same child twice")
