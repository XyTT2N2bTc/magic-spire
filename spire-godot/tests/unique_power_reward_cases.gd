...
static func shop(t) -> void:
 ...
 var before=g.export_snapshot()
 t.check(g.get_view().shop.stock.is_empty() and not g.candidates().any(func(c):return c.payload.kind=="service" and c.payload.get("op","")=="take"),"UNIQUE existing unsold shop card hides when owned")
 t.check(not g.dispatch(stale.id,g.state.version).ok and g.export_snapshot()==before,"UNIQUE hidden stale purchase cannot spend resources")
 g.Cards.remove_permanent(g,card.uid)
