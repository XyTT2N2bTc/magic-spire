...
static func run(t) -> void:
 ...
 for type in ["strain","slip"]:
  ...
  t.check(choice.detail.contains("波及：") and choice.detail.contains(peer.name) and ui.game.state==before,"SPLASH UI formal target preview includes collateral damage without state mutation "+type)
