...
static func run(t) -> void:
 ...
 t.check(button.disabled and t.visible_text(button).contains("仅可使用自身魔力购买") and row.price==90,"MEMBERSHIP UI shows original price and purchase-only payment restriction")
 ...
 t.check(ui.view.mana==10 and ui.find_child("RelicShortcut_membership_card",true,false)!=null,"MEMBERSHIP UI real purchase grants icon after full self payment")
 ...
 t.check(ui.view.mana_flask.mana==500-card.price and ui.view.mana==10,"MEMBERSHIP UI next purchase pays the displayed half price from bottle only")
