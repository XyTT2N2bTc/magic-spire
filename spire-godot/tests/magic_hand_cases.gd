...
static func run(t) -> void:
 ...
 for amount in [10.0,8.0,4.0,1.0]:
  ...
  t.check(not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"HAND stale card refuses all loosening and payment")
  var result=g.dispatch(c.id,g.state.version);var hits=Follow.hits(g)
 ...
 t.check(not blocked.valid and blocked.reason.contains("覆盖") and not g.dispatch(blocked.id,g.state.version).ok and g.state==blocked_before,"HAND cannot bypass a covering outer layer on initial selection")
 for free in [false,true]:
  ...
  t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.state==before,"HAND both faces reject insufficient mana atomically")
  ...
  t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.state==before,"HAND mouth casting obstruction blocks either face before payment")
 ...
 for type in [TYPE,"magic_hand_gift"]:
  for attack in ["strike","heavy"]:
   for form in [0,1]:
    ...
    t.check(g.state==before and not g.dispatch(c.id,g.state.version-1).ok and g.state==before,"HAND readonly and stale requests do not spend charges")
    t.check(g.dispatch(c.id,g.state.version).ok and g.state.card_buff_uses.get("magic_hand_free")==1 and g._enemy(enemy.id).hp==300-c.payload.damage*c.payload.hits and g.state.equipment==equipment,"HAND whole multi-hit attack spends one use and leaves equipment unchanged")
    ...
    t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.state==before,"HAND still requires standing and rejected use preserves buff")
    g.state.posture="stand";g.state.energy=0;c=Attack.attack(t,g,"strike",0);before=g.export_snapshot()
    t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.state==before,"HAND does not waive energy cost")
    ...
    t.check(g.dispatch(c.id,g.state.version).ok and g.state.card_buff_uses.is_empty() and "magic_hand_free" not in g.state.card_buffs and not Attack.attack(t,g,"strike",0).valid,"HAND second use expires and restores real restraint restrictions")
 var g=fresh();var card=Give.give(g,TYPE)
 t.action(g,"card",{"uid":card.uid,"free":true});g.state.energy=10
 var c=Attack.attack(t,g,"kick",1)
 t.check(g.dispatch(c.id,g.state.version).ok and g.state.card_buff_uses.get("magic_hand_free")==2,"HAND leg attacks do not spend uses")
