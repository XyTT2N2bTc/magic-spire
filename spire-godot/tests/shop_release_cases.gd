...
static func run(t) -> void:
 for locked in [false,true]:
  ...
  t.check(g.dispatch(c.id,g.state.version).ok and g._equipment(target.id).is_empty(),"SHOP RELEASE complete ordinary removal")
  ...
  t.check(not g.dispatch(c.id,before.version).ok and g.export_snapshot()==committed,"SHOP RELEASE stale purchase rejected atomically")
 ...
 for spec in [["glove","long"],["leg","upper"],["leg","lower"],["jacket","standard"]]:
  ...
  var before=g.export_snapshot()
  t.check(g.dispatch(c.id,g.state.version).ok,"SHOP RELEASE composite commits "+str(spec))
 ...
 var before=g.export_snapshot()
 t.check(not c.valid and c.reason.contains("外层") and not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SHOP RELEASE covered target cannot charge or remove other roots")
 ...
 t.check(not c.valid and not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SHOP RELEASE insufficient mana preserves all state")
 g.state.mana=20
 ...
 t.check(g.dispatch(c.id,g.state.version).ok and g.action_targets().is_empty(),"SHOP RELEASE attached straps, torso connection fully cleaned")
 ...
 t.check(not g.dispatch(c.id,g.state.version).ok and g.export_snapshot()==before,"SHOP RELEASE post-execution validation failure rolls back payment and removal")
