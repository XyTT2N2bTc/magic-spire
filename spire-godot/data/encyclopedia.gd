...
static func entries(g=null, character: String="") -> Array:
 ...
 for type in N.TYPES:
  ...
  if type=="binding_box":
   var support_hp=N.ENCOUNTERS.iron_man_solo.members.filter(func(member):return member.type=="binding_box")[0].hp
   text="生命：%s；铁男战中的随行实例为%s，小魔女铁男战为%s。\n特性：非魔法伤害减半。\n开场：捕缚40/100，固定坐姿，上身束缚等级至少1。施加后的首个玩家回合不触发；此后每个玩家回合开始施加1件中级2档皮革拘束具，捕缚＋10；无位置仍增加进度。\n行动：随机施加2件中级2档皮革拘束具／口球，或加固皮革共4档→准备→施加1件备用复合拘束具，循环。没有加固目标时只选施加。\n备装：中级2档短上段单腿套、短下段单腿套、露手直肩带单手套各1件。成功施加才消耗，满位可替换，用尽后改为捕缚＋10；捕缚解除不会补充备装。\n铁男战名称：凑数型拘束盒。初始捕缚改为20/100，循环行动1改为施加2件中级1档拘束具或加固共2档；回合开始及备用装备耗尽后的捕缚追加均为＋5，其余效果相同。" % [spec.hp,support_hp,support_hp*1.3]
