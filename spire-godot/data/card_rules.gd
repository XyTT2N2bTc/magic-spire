...
static var BUFFS={
 ...
 "endless_war_goddess":{"name":"无尽魔法少女战神","duration":"battle","attack":"all","ignore_restraints":true,"unrestricted_basics":true,"detail":"本场战斗剩余时间，全部基础动作（包括火球术）无视拘束、快感与姿势限制；所有踢击形态可右键切换。费用、冷却及释放次数照常。"},
...
static var SPECS=_with_hannya({
 ...
 "desire_rune":{"card_type":"power","reward_pool":"lewd_magic","rarity":"rare","cost":1,"mode":"power","self_faces":{"bound":{"pressure_cost":30.0,"buff":"desire_rune_bound"},"free":{"energy_cost":1,"pressure_cost":30.0,"buff":"desire_rune_free"}}},
 "forced_climax":{"card_type":"magic","reward_pool":"lewd_magic","rarity":"uncommon","cost":1,"mode":"self","casting":{"parts":["none"],"multiplier":1.0},"select_exhaust":3,"self_faces":{"bound":{"cast":true},"free":{"cast":true}}},
 "forced_edging":{"card_type":"magic","reward_pool":"lewd_magic","rarity":"uncommon","cost":0,"mode":"self","casting":{"parts":["none"],"multiplier":1.0},"self_faces":{"bound":{"cast":true,"pressure_cost":20.0,"effects":[{"op":"energy","amount":1}]},"free":{"cast":true,"energy_cost":1,"mana_cost":10.0,"effects":[{"op":"draw","amount":2},{"op":"pressure_loss","amount":20}]}}},
 ...
 "binding_power":{"card_type":"skill","rarity":"common","cost":1,"mode":"self","bound_modes":["self","self"],"self_faces":{"bound":{"worn_resource":{"resource":"turn_strength","divisor":2}},"free":{"worn_resource":{"resource":"charge","divisor":4}}}},
 ...
 "boar_emperor_blaze":{"card_type":"skill","rarity":"rare","cost":3,"mode":"strain","damage_type":"strain","base":6.0,"hits":5,"follow_through":true,"follow_through_scope":"body","target_slots":FOLLOW_THROUGH_SLOTS,"free_effects":[{"op":"charge","amount":6}]},
...
const COMMON=["self_satisfaction","psychological_suggestion","rally_spirit","binding_power","kip_up","prepared_chant","confluence","leverage","breath_control","siphon","ready_to_strike","crossed_legs","mana_search","repeated_strain","embers","brace","inch","strong_elbow","mana_surge","mana_invocation","rekindle"]
const UNCOMMON=["forced_edging","forced_climax","supple_flesh","mana_attachment","binding_search","self_binding","reuse","resonance","formation","magic_hand","flame_flourish","wildfire_descent","restraint_embrace","pot_of_greed","concentration","focus","tear","chain","peel","unlock","mana_conversion","letter_opener","adaptability","echo_cast","fire_control"]
const RARE=["desire_rune","sympathetic_form","shared_fate","siphon_strength","practiced","light_as_swallow","infusion","mana_circuit","boar_emperor_blaze","fire_mastery","binding_enthusiast","pleasure_conversion","henshin","fire_dynamics","endless_war_goddess"]
 ...
 if spec.mode in ["self","power"]:
  ...
  for side in ["bound","free"]:
   ...
   if face.has("worn_resource"):
    ...
    if spec.mode!="self" or not gain is Dictionary or gain.keys().any(func(key):return key not in ["resource","divisor"]) or gain.get("resource","") not in ["mana","turn_strength","charge"] or not gain.get("divisor") is int or gain.divisor<1: return "佩戴件数收益需要合法资源及正整数除数。"
