...
弱怪入场按当前装备过滤并累计到总强度2；strength允许1或2，可为两只强度1或一只强度2，允许同种重复，强度不在前台显示。口部占用排除口球，无未锁可上锁装备排除漂浮锁。完整筛选与回退见 `docs/design/enemies-first-floor.md`。漂浮锁只做预告／上锁循环，不会生成其他装备。内置杂乱拘束具的加权行动仍由代码配置，本JSON模板不新增动作编辑接口。
